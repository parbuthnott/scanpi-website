...
/* alternative versions of read in order to get over 502 gateway errors from mbrainz? 
//		$mbrainzhandle = fopen($mbrainzurl, "r");
//		$mbrainz = fread($mbrainzhandle, filesize($mbrainzurl));
//		fclose($mbrainzhandle);

//		$handle = fopen($mbrainzurl, "rb");
//		$mbrainz = stream_get_contents($handle);
//		fclose($handle);
...
//		$mbrainz = '';
//		while (!feof($handle)) {
//		    $mbrainz .= fread($handle, 8192);
//		}
//		fclose($handle);

//		$mbrainz = file_get_contents("http://www.google.com/?".$mbrainzurl."SEE");
 end alt versions */
                $mbrainz = file_get_contents($mbrainzurl);
		if (FALSE === $mbrainz) {
			echo "<h4>Hmmm, no good response from Musicbrainz call (".$mbrainzurl.")</h4>";
		} else {
			echo "<!-- mbrainz response is not false ... which bodes well ...-->";
		}
               echo "<!-- $mbrainz -->";
      /*
               // example response...
